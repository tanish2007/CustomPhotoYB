"""
SigLIP embeddings for photo clustering.
SigLIP (Sigmoid Loss for Language-Image Pre-training) often provides
better fine-grained visual understanding than CLIP.

Uses google/siglip-base-patch16-224 by default (768-dim embeddings)
"""
...
# Try to import transformers for SigLIP
try:
...
class SigLIPEmbedder:
    """Generate SigLIP embeddings for photos."""
    ...
    def __init__(self, model_name: str = "google/siglip-base-patch16-224", device: str = None):
        ...
        Args:
            model_name: SigLIP model variant. Options:
                - "google/siglip-base-patch16-224" (768-dim, balanced)
                - "google/siglip-large-patch16-384" (1024-dim, better quality)
                - "google/siglip-so400m-patch14-384" (1152-dim, best quality)
        ...
        print(f"SigLIP loaded. Embedding dimension: {self.embedding_dim}")
    ...
    def get_embeddings_batch(self, images: List[Image.Image], batch_size: int = 16) -> np.ndarray:
        """Get SigLIP embeddings for a batch of images."""
        all_embeddings = []
    ...
    def save_embeddings(self, embeddings: Dict[str, np.ndarray],
        ...
        print(f"Saved SigLIP embeddings to {output_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python siglip_embeddings.py <folder_path>")
        print("\nThis will generate SigLIP embeddings for all images in the folder.")
        sys.exit(0)
    ...
    embedder.save_embeddings(embeddings, os.path.join(output_dir, "siglip_embeddings.npz"))
